...
class WorkshopChatbot {
  ...
  formatMessage(content) {
    ...
    // AsciiDoc section headers (handle trailing == properly)
    ...
    // AsciiDoc bullet lists (protect from bold processing by replacing * with placeholder)
    formatted = formatted.replace(/^\*\s+(.+?)$/gm, function(match, content) {
      return `<li class="bullet-list">${content}</li>`;
    });
    ...
    // Bare URLs - convert standalone URLs to clickable links (display without scheme)
    formatted = formatted.replace(/(?<![\["])(https?:\/\/[^\s\[\]()<>]+)(?![\]"])/g, function(match) {
      const displayText = match.replace(/^https?:\/\//, '');
    ...
    // Single asterisks for bold (but avoid line-start asterisks which are bullet lists, and square brackets)
